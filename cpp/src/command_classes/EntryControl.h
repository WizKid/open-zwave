...
namespace OpenZWave
{
	namespace Internal
	{
		namespace CC
		{
			...
			/** \brief Implements COMMAND_CLASS_ENTRY_CONTROL (0x6f), a Z-Wave device command class.
			 * \ingroup CommandClass
			 */
			class EntryControl: public CommandClass
			{
				public:
					static CommandClass* Create(uint32 const _homeId, uint8 const _nodeId)
					{
						return new EntryControl(_homeId, _nodeId);
					}
					virtual ~EntryControl()
					{
					}
					...
					{
						return "COMMAND_CLASS_ENTRY_CONTROL";
					}
				...
				private:
					EntryControl(uint32 const _homeId, uint8 const _nodeId) :
