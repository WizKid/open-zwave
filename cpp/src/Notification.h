	...
	class OPENZWAVE_EXPORT Notification
	{
		...
		public:
			...
			enum NotificationType
			{
				...
				Type_EntryControl /**< A EntryControl Notification message containing the sequence number, event type and event data */
			};
			...
			{
				assert(Type_NodeEvent == m_type || Type_ControllerCommand == m_type || Type_EntryControl == m_type);
				return m_event;
			}
		...
		private:
			...
			void SetSequenceNumber(uint8 const sequenceNumber) {
				assert(Type_EntryControl == m_type);
				m_byte = sequenceNumber;
			}
